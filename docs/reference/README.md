...
- [Channel types](channels.md) — the `spsc` / `mpsc` / `sharded` matrix:
  producers, ordering guarantees, capacity rules, feature flags, handle
  rules.
- [Wait strategies](wait-strategies.md) — the four variants with measured
  idle-CPU and wake-latency figures, ladder constants, and provenance.
- [Errors and disconnect semantics](errors-and-disconnect.md) — every error
  type, the disconnect matrix, and what happens to values left in the ring.
